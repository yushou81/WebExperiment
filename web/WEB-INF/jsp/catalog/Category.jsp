...
<div class="book-slide">
	<div class="book js-flickity" data-flickity-options='{ "wrapAround": true }'>
		<c:forEach var="product" items="${sessionScope.productList}">
			<div class="book-cell">
				<div class="book-img">
					<img data-product-id="${product.productId}" src="${product.img}" alt="${product.name}" class="book-photo" />
					<div class="tooltip" id="tooltip-${product.productId}">
						<p><strong>Product Name:</strong> ${product.name}</p>
						<p><strong>Author:</strong> ${product.productId}</p>
				...
				<div class="book-content">
					...
					<div ><button id="bookss" class="book-see" onclick="getItemList('${product.productId}')"> See The ${product.categoryId}</button></div>
				</div>
			</div>
		</c:forEach>
	</div>
				...
				$.each(response, function (index, item) {
					...
					var addToCartButton = $('<button>')
							...
							.on('click', function () {
								...
								$.ajax({
									url: "addItemToCart", // Servlet 的路径
									method: "GET",
									data: { workingItemId: item.itemId }, // 将 itemId 作为请求参数
									success: function (response) {
					...
					// 将所有内容添加到卡片内容 div 中
					cardContent.append(bookName, bookBy, rate, bookSum,addToCartButton);
...
<script src="js/FlowWindow.js"></script>
</html>
