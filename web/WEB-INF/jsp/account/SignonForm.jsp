	...
	<div class="container" id="container">
		<div class="form-container sign-up-container">
			<form action="newAccount" method="post">
				...
				<input type="text" name="username" placeholder="Username" onblur="checkUsername(this.value);"/>
				<span id="isExistInfo2" class=""></span>
				<input type="email" name="email" placeholder="Email" onblur="checkEmail(this.value);" />
				<span id="isEmail2" class=""></span>
				<input type="password" name="password" placeholder="Password" onblur="checkPassword(this.value);" />
				<span id="isPassword2" class=""></span>
				<input type="password"  name="repeatedPassword" placeholder="Confirm Password" onblur="checkRepeatPassword(this.value);"/>
				<span id="isRepeatPassword2" class=""></span>
				<input type="text" name="vCode" size="5" maxlength="4" placeholder="VerificationCode" onblur="checkVCode(this.value);"/>
		...
		<div class="form-container sign-in-container">
			<form action="signOn" method="post">
				...
				<input type="text" name="username" value="uu" onblur="checkUsername(this.value);" placeholder="username"/>
				<span id="isExistInfo1" class=""></span>
				<input type="password" name="password" value="12" placeholder="password" onblur="checkPassword(this.value)"/>
				<input type="text" name="vCode" size="5" maxlength="4" placeholder="VerificationCode" onblur="checkVCode(this.value)/>
				<a href="signOn"><img border="0" src="verificationCode" name="checkcode"></a>
...
<script>
	...
	function checkUsername(username){
		...
		if(username == ""){
			...
			var msg2 = document.getElementById('isExistInfo2');
			msg.innerHTML='';
			msg2.innerHTML='';
			return;
		}
	...
	function checkEmail(email) {
		...
		// 获取显示验证信息的元素
		var msg = document.getElementById('isEmail2');
		...
		if (emailPattern.test(email)) {
			msg.innerHTML = ''; // 清除错误提示
			// 可以在这里执行其他逻辑，比如标记邮箱有效
		} else {
			msg.innerHTML = '请输入有效的邮箱地址'; // 显示错误信息
			msg.classList.add("errormsg");
			msg.classList.remove("okmsg");
		}
	}
	...
	function checkPassword(password){
		...
		if(password == ""){
			var msg = document.getElementById('isPassword2');
			msg.innerHTML='';
			return;
		}
	}
	...
	function checkRepeatPassword(repeatPassword){
		...
		if(repeatPassword == "" && password == ""){
			var msg = document.getElementById('isRepeatPassword2');
			msg.innerHTML='';
			return;
		}else if(repeatPassword == "" && password != ""){
			var msg = document.getElementById('isRepeatPassword2');
			msg.classList.add("errormsg");
			msg.innerText='请输入确认密码';
			msg.classList.remove("okmsg");
		}else if(repeatPassword != "" && password == ""){
			var msg = document.getElementById('isRepeatPassword2');
			msg.classList.add("errormsg");
	...
	function checkVCode(vCode){
		...
		$(".sign-up-btn").click(function(e){
			if(username==''|| email=='' ||password == ''|| repeatPassword == ''|| vCode == ''){
				console.log("fail")
				console.log(username+" "+email+" "+password+" "+repeatPassword+" "+vCode)
				e.preventDefault();
			}
		})
