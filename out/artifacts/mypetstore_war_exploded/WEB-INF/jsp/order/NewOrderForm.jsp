	...
	<form action="conFirmOrderForm" method="post">
	    <table id="mainTable">
			...
			<tr>
				<td>Card Number:</td>
				<td>
					<input type="text" id="creditCard" name="creditCard" value="999999999999" />
					<span id="cardError" style="color: red; display: none;">* Use a fake number!</span>
				</td>
			</tr>
			<tr>
		...
		<input type="submit" name="newOrder" value="Continue" class="btn-a" />
	</form>
</div>
...
<script>
    ...
    `;
			table.insertAdjacentHTML("beforeend", shippingRows);
		} else {
			// 如果复选框未选中，移除所有插入的行
			const rowsToRemove = table.querySelectorAll(".shippingRow, #shippingRowHeader");
...
<%--验证信用卡--%>
<script>
	// 获取 Card Number 输入框和错误提示元素
	const creditCardInput = document.getElementById("creditCard");
	const cardError = document.getElementById("cardError");

	// 定义 Card Number 格式的正则表达式（仅接受 16 位数字）
	const cardNumberRegex = /^\d{12,16}$/;
