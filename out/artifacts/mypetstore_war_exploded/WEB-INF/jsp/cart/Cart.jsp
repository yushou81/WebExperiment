...
<%--		<button onclick="newOrderForm()">go to pay</button>--%>
<%--		<button onclick="renewCart('${sessionScope.account.username}',1,1)">Update Cart</button>--%>
		<a class="Go-to btn-a doNotDisplay" href="newOrderForm">Go to Pay</a>
		<a class="Return btn-a" href="main">Go to Add</a>
	</div>
...
<script>
	...
	function displayCart(cart) {
		...
		//如果没有就显示空的tr
		var emptyTr = $('<tr>');
		var emptyTd = $('<td>').attr('colspan', 8).text('Your cart is empty.')
		emptyTr.append(emptyTd);
		if(cartItemsList.length === 0) {
			$('.cartItems').append(emptyTr);
			$('.Go-to').addClass('doNotDisplay');
			$('.Return').removeClass('doNotDisplay');
			return;
		} else {
			$('.Go-to').removeClass('doNotDisplay');
			$('.Return').addClass('doNotDisplay');
		}
